...
class QrctWp_from_Goaskle_Com
{
    ...
    private $defaultOptions = array(
        'widget' => array(
            ...
            'qrcode_goaskle_access_level' => 'administrator',
        'shortcode' => array(
            'size' => '150',
            'enc' => 'UTF-8',
            'ecc' => 'L',
...
function goaskle_my_custom_css_for_plugins_list_in_admin() {
  echo '<style>body.wp-admin table.plugins tr[data-slug="qr-code-tag-for-wc-from-goaskle-com"] .plugin-title.column-primary > strong{white-space: break-spaces !important;}</style>';
}
add_action('admin_head', 'goaskle_my_custom_css_for_plugins_list_in_admin');
...
//goaskle.com 27 05 23 support of plugin WooCommerce PDF Invoices, Packing Slips, Delivery Notes and Shipping Labels plugins ( By WebToffee, Emails + Files )
if ( class_exists( 'Wf_Woocommerce_Packing_List' ) ) {
/*if ( (isset($wp->query_vars['print_packinglist']) && $wp->query_vars['print_packinglist'] == 'true') || (isset($_GET['print_packinglist']) && $_GET['print_packinglist'] == 'true') ){*/

add_filter( 'wf_pklist_alter_footer_data', 'goaskle_qr_code_for_pklist_footer', 777, 1 );

function goaskle_qr_code_for_pklist_footer ($footer_data){
//print_r($order);
$align = 'center';
if ($_GET['type'] == 'print_shippinglabel'){
//$align = 'right';
}
	$footer_data .= "<p style='text-align: ".$align.";'>".do_shortcode('[qrcodetag_from_goaskle_com/]')."</p>";
	return $footer_data;
}/*}*/}
//goaskle.com 27 05 23 support of plugin WooCommerce PDF Invoices, Packing Slips, Delivery Notes and Shipping Labels plugins ( By WebToffee, Emails + Files )
...
if (isset(get_option('qrct_from_goaskle_com_options')['shortcode']['qrcode_goaskle_access_level'])){
$trimed_access_level = preg_replace('/\s+/', '', get_option('qrct_from_goaskle_com_options')['shortcode']['qrcode_goaskle_access_level']);
$current_user_can_do = explode(",",$trimed_access_level);

$user = wp_get_current_user();
if ( array_intersect( $current_user_can_do, $user->roles ) ) {
   $is_admin = 1;
add_filter( 'woocommerce_order_received_verify_known_shoppers', '__return_false' );   
    ...
    $current_user_can_do = "administrator";
}
...
exit;
	...
	add_action( 'woocommerce_before_thankyou', 'goaskle_com_qr_code_custom_thank_you_valid' );
		//add_action( 'woocommerce_endpoint_order-received_title', 'goaskle_com_qr_code_custom_thank_you_valid' );
...
$hasd_status_text = '<p id="hashstatus" class="hashstatus_text active" name="hashstatus_active">'.__( 'Hash status:', 'qr-code-tag-for-wc-from-goaskle-com' )." ".__( 'Active', 'qr-code-tag-for-wc-from-goaskle-com' ).'</p>';  
...
$hasd_status_text = '<p id="hashstatus" class="hashstatus_text deactivated" name="hashstatus_restore">'.__( 'Hash status:', 'qr-code-tag-for-wc-from-goaskle-com' )." ".__( 'Deactivated', 'qr-code-tag-for-wc-from-goaskle-com' ).'</p>';  
}
...
com = '0';
   ...
   var butt_for_activate = '<button type="submit" id="make_used_qr_code" name="make_used_qr_code_use" href="#" class="butt-gaqr-adm butt-act-gaqr-adm button button-default">'+'<?php echo __( "Activate hash", "qr-code-tag-for-wc-from-goaskle-com" ); ?>'+'</button>';
	var hashstatus_for_active = '<p id="hashstatus" class="hashstatus_text active" name="hashstatus_active">'+ '<?php echo __( 'Hash status:', 'qr-code-tag-for-wc-from-goaskle-com' ); ?>'+' '+ '<?php echo __( 'Active', 'qr-code-tag-for-wc-from-goaskle-com' ); ?>'+'</p>';
   var butt_for_deactivate = '<button type="submit" id="make_used_qr_code" name="make_used_qr_code_restore" href="#" class="butt-gaqr-adm butt-deact-gaqr-adm button button-default">'+'<?php echo __( "Cancel activation", "qr-code-tag-for-wc-from-goaskle-com" ); ?>'+'</button>';	
	var hashstatus_for_deactivated = '<p id="hashstatus" class="hashstatus_text deactivated" name="hashstatus_restore">'+ '<?php echo __( 'Hash status:', 'qr-code-tag-for-wc-from-goaskle-com' ); ?>'+' '+ '<?php echo __( 'Deactivated', 'qr-code-tag-for-wc-from-goaskle-com' ); ?>'+'</p>';
	
	switch(res.res){
		case 'activated':
		document.getElementsByClassName('butt-act-gaqr-adm')[0].outerHTML = butt_for_deactivate;
		document.getElementsByClassName('hashstatus_text')[0].outerHTML = hashstatus_for_deactivated;		
      ...
      document.getElementsByClassName('butt-gaqr-adm')[0].disabled = false;
		...
		document.getElementsByClassName('hashstatus_text')[0].outerHTML = hashstatus_for_active;		
...
<style>
.hashstatus_text.deactivated{
    color: red;
    font-weight: bold;
    font-size: 20px;
}
.hashstatus_text.active{
    color: green;
    font-weight: bold;
    font-size: 20px;
}
...
.validated_thanks_qr_code_goaskle{
    ...
    color: green;
        ...
        <div class="validated_thanks_qr_code_goaskle_desc_second"><?php _e( "HAVE A NICE DAY!", "qr-code-tag-for-wc-from-goaskle-com" ); ?></div>
                <div class="validated_thanks_qr_code_goaskle_desc_thank"><?php _e( "THANK YOU", "qr-code-tag-for-wc-from-goaskle-com" ); ?></div>
                <div><?php echo $hash_code; ?></div>
                <div><?php echo $hasd_status_text; ?></div>                
                <hr><br>
			</div>
			<?php if ($is_admin) { ?>
...
add_filter( 'woocommerce_email_footer', 'goaskle_com_filter_woocommerce_email_footer' );
 ...
 if ($disable_auto_wc == 0 && $disable_all_wc_emails_except_standard == 1) {
    ...
    add_action( 'woocommerce_email_footer', 'goaskle_com_add_custom_text_to_new_order_email' );
    
        function goaskle_com_add_custom_text_to_new_order_email( $email ) {
        // Only for "New Order"
        //print_r($email);
        if ($email){
				$sent_to_admin = (strpos($email->template_plain, 'admin')) ? 1 : 0;        	
        } else {
        		$sent_to_admin = 1;
        		return;
        }
...
if ($email->id <> 'order_email_to_vendor'){
//add_filter( 'woocommerce_email_footer_text', '__return_empty_string', 20 );
   echo wp_kses_post("<div style='text-align: center;'>".do_shortcode('[qrcodetag_from_goaskle_com/]')."</div>");
} else {
//add_filter( 'woocommerce_email_footer_text', 'goaskle_com_filter_woocommerce_email_footer_text' );
}
...
function goaskle_com_wpo_wcpdf_after_footer($type,$ordersun) {
    $ids = $ordersun->get_id();
   echo wp_kses_post("<div style='text-align: right;'>".do_shortcode('[qrcodetag_from_goaskle_com ids="'.$ids.'"/]')."</div>");
}
...
add_action( 'woocommerce_view_order', 'goaskle_woocommerce_view_order_action',777 );
}
...
function goaskle_woocommerce_view_order_action( $order_id ){
   echo wp_kses_post("<div style='text-align: center;'>".do_shortcode('[qrcodetag_from_goaskle_com/]')."</div>");
}
...
if (isset($_POST['widget_qrct_from_goaskle_com-before']) && isset($_POST['widget_qrct_from_goaskle_com-after'])) {
    ...
    */
    public function shortcode($atts, $content = NULL) 
    {
      ...
      global $woocommerce;
//echo "<pre>";print_r('tut00');echo "</pre>"; //qq       
if (isset(get_option('qrct_from_goaskle_com_options')['shortcode']['disable_auto_wc'])){
$disable_auto_wc = get_option('qrct_from_goaskle_com_options')['shortcode']['disable_auto_wc'];	
} else {
	$disable_auto_wc = 0;
...
//echo "<pre>";print_r('tut00-0');echo "</pre>"; //qq       

if ( (isset($wp->query_vars['view-order']) || isset($wp->query_vars['order-received']) || isset($_GET['order_ids']) ) && !isset($wp->query_vars['print-order']) && !isset($_GET['print-order']) && !isset($wp->query_vars[get_option( 'wcdn_print_order_page_endpoint', 'print-order' )])){
//echo "<pre>".'tut0'."</pre>"; //qq
        ...
        if (isset($_GET['order_ids']) && $_GET['order_ids'] > 0) {
               ...
               $wcid = (isset($wp->query_vars['order-received'])) ? absint($wp->query_vars['order-received']) : '';
    	  }
    	  if (!$wcid){
    	  	      $wcid = absint($wp->query_vars['view-order']);   
    	  }
    	  if (isset($atts['ids']) && isset( $_REQUEST['bulk'] ) && function_exists('WPO_WCPDF') && null !== WPO_WCPDF() && strpos($_GET['order_ids'],"x")){
    	      $wcid = $atts['ids'];
    	  }
...
//echo "<pre>".'tut1'."</pre>"; //qq


    $wc_ord_numb = $order->get_order_number();
    $wc_ord_bil_phone = $order->get_billing_phone();
    $wc_ord_bil_email = $order->get_billing_email();

$goa_get_customer_id = $order->get_customer_id();
$goa_get_user_id = $order->get_user_id();
    
//10 05 23 coupons goaskle
$output = array(); // 
$output_names = array(); // 
$output_sum = array(); // 
...
foreach( $order->get_items('coupon') as $item_id => $item ){
    ...
    $output_names[] = $data['code'];
    $output_sum[] = strip_tags(wc_price($data['discount'] + $data['discount_tax']));
}

$goa_coupons_names_plus_sums = implode(', ', $output);
$goa_coupons_sums = implode(', ', $output_sum);
$goa_coupons_names = implode(', ', $output_names);
$goa_coupons_total = $order->get_discount_total();
...
//10 05 23 coupons goaskle
    ...
    $goa_wc_dat_tem = ($options && isset($options['wc_data_template'])) ? $options['wc_data_template'] : '';
    $goa_domain = get_site_url();

//070523 map and ship address goaskle    
    if ($order->get_formatted_shipping_address()) {
    $goa_ship_address = nl2br($order->get_formatted_shipping_address());
    $goa_ship_address = preg_replace("/<br\W*?\>/", " ", $goa_ship_address); 
    $goa_ship_address = trim(preg_replace("/".$order->get_formatted_shipping_full_name()."/", "", $goa_ship_address));     
  	 $goa_map_ship_address = $order->get_shipping_address_map_url();
  	 
    } else {
    	 if ($order->get_formatted_billing_address()){
    $goa_ship_address = nl2br($order->get_formatted_billing_address());
    $goa_ship_address = preg_replace("/<br\W*?\>/", " ", $goa_ship_address); 
    $goa_ship_address = trim(preg_replace("/".$order->get_formatted_billing_full_name()."/", "", $goa_ship_address));     
  	 $goa_map_ship_address = urlencode("https://maps.google.com/maps?q=".$goa_ship_address);
    	 } else {
    	 	$goa_ship_address = '';
    	 	$goa_map_ship_address = '';
    	 }
    }
//070523 map and ship address goaskle    
...
//070523 map and ship address goaskle    
    ...
    $goa_ship_address = preg_replace("/<br\W*?\>/", " ", $goa_ship_address); 
    $goa_ship_address = trim(preg_replace("/".$order->get_formatted_billing_full_name()."/", "", $goa_ship_address));     
  	 $goa_map_ship_address = "https://maps.google.com/maps?q=".$goa_ship_address;
    	 } else {
    	 	$goa_ship_address = '';
    	 	$goa_map_ship_address = '';
    	 }
...
//28 05 23 goaskle.com aupport WC product vendors plugin _no qr code for vendors

if ( class_exists('WC_Product_Vendors_Utils') ) {
$this_email_is_vendor = array();

$goa_cust_email = $order->get_billing_email();
$vendors = WC_Product_Vendors_Utils::get_vendors_from_order( $order );        

if ($vendors && is_array($vendors) && count($vendors) > 0 ) {
	
				foreach ( $vendors as $vendor_id => $data ) {

					$vend_email = ! empty( $data['email'] ) ? $data['email'] : '';

					if ( empty( $vend_email ) ) {
//					$this_email_is_vendor = 0;
						continue;
					}
					if ($vend_email == $goa_cust_email){
					$this_email_is_vendor = 0;
					} else {
						$this_email_is_vendor[] = 1;
					}
...
//print_r(WC()->mailer()->get_emails()['WC_Email_New_Order']->recipient);
//print_r($koko);
}
//28 05 23 goaskle.com aupport WC product vendors plugin    
...
//goaskle.com 27 05 23 invoice    	
   } elseif ( class_exists( 'Wf_Woocommerce_Packing_List' ) && $_GET['post'] && ((isset($wp->query_vars['print_packinglist']) && $wp->query_vars['print_packinglist'] == 'true') || (isset($_GET['print_packinglist']) && $_GET['print_packinglist'] == 'true')) ){

//echo "<pre>".'tut10'."</pre>"; //qq       
...
if ($_GET['type'] == 'print_shippinglabel'){
    $options['size'] = 100;	
                ...
                if (is_numeric($_GET['post'])){
	               $wcid = (int)sanitize_text_field($_GET['post']);                	
                } else {
        					$wcid = Wf_Woocommerce_Packing_List::wf_decode($_GET['post']);        	
                }
...
if (isset($wcid) && $wcid > 0){
 ...
 //120923 - fix shortcode in template goaskle.com

 $goa_wc_dat_tem = str_replace('[qrcodetag_from_goaskle_com/]', '', $goa_wc_dat_tem);  //replace shortcode
 $goa_wc_dat_tem = str_replace('[qrcodetag_from_goaskle_com ]', '', $goa_wc_dat_tem);  //replace shortcode 
 $goa_wc_dat_tem = str_replace('[qrcodetag_from_goaskle_com]', '', $goa_wc_dat_tem);  //replace shortcode
 ...
 //120923 - fix shortcode in template
 ...
 $goa_wc_dat_tem = str_replace('{customer_phone}', $wc_ord_bil_phone, $goa_wc_dat_tem);  //replace {customer_phone} //
 $goa_wc_dat_tem = str_replace('{customer_email}', $wc_ord_bil_email, $goa_wc_dat_tem);  //replace {customer_email} // 
 
 $goa_wc_dat_tem = str_replace('{wc_order_customer_id}', $goa_get_customer_id, $goa_wc_dat_tem);  //replace {wc_order_customer_id} //197
 $goa_wc_dat_tem = str_replace('{wc_order_user_id}', $goa_get_user_id, $goa_wc_dat_tem);  //replace {wc_order_user_id} //197
...
//070523 address
 $goa_wc_dat_tem = str_replace('{order_ship_address}', $goa_ship_address, $goa_wc_dat_tem);  //replace {order_ship_address}
 $goa_wc_dat_tem = str_replace('{order_map_ship_address}', $goa_map_ship_address, $goa_wc_dat_tem);  //replace {order_ship_address} 

//10 05 23 coupon
 $goa_wc_dat_tem = str_replace('{wc_coupons_names_plus_sums}', $goa_coupons_names_plus_sums, $goa_wc_dat_tem);  //replace 
 $goa_wc_dat_tem = str_replace('{wc_coupons_sums}', $goa_coupons_sums, $goa_wc_dat_tem);  //replace 
 $goa_wc_dat_tem = str_replace('{wc_coupons_names}', $goa_coupons_names, $goa_wc_dat_tem);  //replace  
 $goa_wc_dat_tem = str_replace('{wc_coupons_total}', $goa_coupons_total, $goa_wc_dat_tem);  //replace   
...
//        if (isset($this_email_is_vendor) && is_array($this_email_is_vendor) && count($this_email_is_vendor) > 0 ) {
...
//filter_input(INPUT_POST,$varia, FILTER_SANITIZE_STRING);
    ...
    */
    public function adminSettingsPage()
    {
      ...
      $options['shortcode']['qrcode_goaskle_access_level'] = sanitize_text_field($_POST['qrct_from_goaskle_com_sc_access_level']);      
