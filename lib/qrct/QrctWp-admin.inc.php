<?php
    ...
    if (isset($options['shortcode']['qrcode_goaskle_access_level']) && $options['shortcode']['qrcode_goaskle_access_level']){
    	$qrcode_goaskle_access_level = $options['shortcode']['qrcode_goaskle_access_level'];
    } else {
...
jQuery('input[name=qrct_from_goaskle_com_sc_customer_invoice_wc_emails_enable]').prop('checked',false);                   
    ...
    <td>
        <select id="qrct_from_goaskle_com_generator" name="qrct_from_goaskle_com_generator" style="width:150px;">
    ...
    <td>
        <select id="qrct_from_goaskle_com_imagetype" name="qrct_from_goaskle_com_imagetype" style="width:60px;">
...
<!-- 26 10 2024 goaskle.com admin control-->      
      <tr>
        <td width="100"><?php echo __('Access level for hash', 'qr-code-tag-for-wc-from-goaskle-com'); ?></td>
        <td colspan="3" width="100"><input type="text" name="qrct_from_goaskle_com_sc_access_level" style="width:100%;" value="<?php echo esc_html($qrcode_goaskle_access_level); ?>" /></td>
        <td><?php echo __('For use in hash feature. Use any capabilities for access level. Comma separated. It will use current_user_can function. Example: administrator,editor. No spaces', 'qr-code-tag-for-wc-from-goaskle-com'); ?><hr></td>
      </tr>
...
<!-- 17 02 2023 goaskle.com adding checkbox for manually block qr code on all pages of woocommerce and keep on emails only -->      
      ...
      <tr>
        <td><b><?php echo __('Disable for ALL Woocommerce emails except these statuses:', 'qr-code-tag-for-wc-from-goaskle-com'); ?></b></td>
        ...
        <td><?php echo __('Disable QR code Generating for All Woocommerce emails, except ( Except these ones: new order email, failed order email, order-on-hold email, processing email, complete email, refunded email ) some statuses', 'qr-code-tag-for-wc-from-goaskle-com'); ?><hr></td>
      </tr>
...
<!-- 11 01 2023 goaskle.com -->      
      ...
      <tr>
        ...
        <td><?php echo __('<b style="color:red;">!!! CLEAR IT FOR AUTOMATIC DATA TEMPLATE IN WOOCOMMERCE !!!</b> Woocommerce will use this template for the data to generate the QR code. The QR code will be created based on the texts and variables of this template. For example (AUTOMATIC):<br>Ivan Ivanov<br>TOTAL AMOUNT $39.10<br>VAT AMOUNT $5.10<br>VAT NUMBER 1235ABN457<br>DATE 03/11/2021<br>TIME 19:03<br>https://goaskle.com<br>You can easy use your own text, labels and these variables: {order_id}, {customer_phone}, {customer_email}, {full_customer_name}, {total_amount}, {total_amount_no_curr}, {vat_amount}, {vat_number}, {order_date}, {order_time}, {wc_website}, {wc_order_items_name1}, {wc_order_items_qty1}, {wc_order_items_price1}, {wc_order_items_subtot1}, {wc_order_items_name2}, {wc_order_items_qty2}, {wc_order_items_price2}, {wc_order_items_subtot2}, {wc_order_items_template1}, {hash_gaqr}, {order_ship_address}, {order_map_ship_address}<br>{wc_order_customer_id}<br>{wc_order_user_id}<br><b>EXAMPLE FOR YOUR OWN TEMPLATE:</b><br>Order ID: {order_id}<br>{full_customer_name}<br>TOTAL AMOUNT: {total_amount}<br>VAT AMOUNT: {vat_amount}<br>VAT NUMBER: {vat_number}<br>DATE {order_date}<br>TIME {order_time}<br>{wc_website}<br>{wc_order_items_template1}<br>{order_ship_address}<br>{order_map_ship_address}<br>{wc_coupons_names_plus_sums}<br>{wc_coupons_sums}<br>{wc_coupons_names}<br>{wc_coupons_total}<br>{wc_order_customer_id}<br>{wc_order_user_id}', 'qr-code-tag-for-wc-from-goaskle-com'); ?><hr></td>
      </tr>      
      <tr>
